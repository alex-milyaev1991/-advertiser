<template>
  <div class="inner-page">
    <div class="flex justify-content-end px-5 mb-3">
      <Button class="mr-3" as="router-link" to="/creatives/create" size="small"
        >Create Creative</Button
      >
      <Button
        as="router-link"
        :to="`/campaigns/${campaignId}/creatives/add`"
        size="small"
        >Add Creative</Button
      >
    </div>
      ...
      >
        ...
        <Column header="Status">
          <template #body="{ data }">
            <ToggleSwitch
              v-model="data.active"
              @change="toggleCreativeStatus(data.id, data.active)"
            ></ToggleSwitch>
          </template>
        </Column>
...
const creatives = ref<Creative[]>(campStore.creatives);
...
const fetchCreatives = async () => {
  ...
  creatives.value = campStore.creatives;
...
const activateCreativeInCampaign = (campId: number, creativeId: number) => {
  campStore.activateCreativeInCampaign(campId, creativeId);
};

const inactivateCreativeInCampaign = (campId: number, creativeId: number) => {
  campStore.inactivateCreativeInCampaign(campId, creativeId);
};

const toggleCreativeStatus = (creativeId, isActive) => {
  const numericCampaignId = Number(campaignId.value);
  if (isActive) {
    activateCreativeInCampaign(numericCampaignId, creativeId);
  } else {
    inactivateCreativeInCampaign(numericCampaignId, creativeId);
  }
};
